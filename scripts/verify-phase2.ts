...
console.log('\nEmitting offline preview (public/phase2-preview.html)')
{
  ...
  const choreoMod = skeletonFor('scrub-choreography')({
    id: 'choreo', kind: 'showcase', copy: sampleCopy('choreo'),
    theme, params: clampParams({ intent: 'scrub-choreography', params: {} }).params,
  })
  const trackMod = skeletonFor('horizontal-scroll-track')({
  ...
  const shiftMod = skeletonFor('theme-shift')({
    id: 'shift', kind: 'showcase', copy: sampleCopy('shift'),
    theme, params: clampParams({ intent: 'theme-shift', params: {} }).params,
  })
